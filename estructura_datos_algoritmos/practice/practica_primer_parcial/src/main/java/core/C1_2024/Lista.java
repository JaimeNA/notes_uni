...
public class Lista{
	...
	// Complejidad temporal: T(N) = 10 + N + N*8 => Sera O(N)
	// Complejidad espacial: T(N) = N + 2 => Sera O(N)
	private  Lista[] randomSplitListas( Integer nLists ) {
		...
		int listNumber = getRandom(nLists);

		while (current != null) {

			if (toReturn[listNumber].first == null) {	// Si no esta initializada
				toReturn[listNumber].first = toReturn[listNumber].last = current;
			} else {
				toReturn[listNumber].last.next = current;
				toReturn[listNumber].last = toReturn[listNumber].last.next; 	// Update the last position
			}
			
			current = current.next;	// Goto next node
			toReturn[listNumber].last.next = null;	// Make it stop pointing to the old list
			listNumber = getRandom(nLists);	// Fetch next list
		}
		first = last = null;
		return toReturn;
	}
