...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    private T removevj(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            // internal node
            Node<T> lesser = node.getChild(index);
            Node<T> lesserDecendant = this.getLesserNode(lesser);
            T replaceValue = this.removeLesserValue(lesserDecendant);
            node.addKey(replaceValue);
            if (lesserDecendant.parent != null && lesserDecendant.numberOfKeys() < minKeySize) {
                this.combined(lesserDecendant);
            }
            if (lesserDecendant.numberOfChildren() > maxChildrenSize) {
                this.split(lesserDecendant);
            }
        }
